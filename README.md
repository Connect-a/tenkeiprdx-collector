...
- キーボード … スペース / → で送り、← で戻る（画面クリックと同じ動作）
- 話の最後まで送ると、次の話があれば「続けて再生しますか？」と確認します（スペースで再生／←→で選んでEnter／クリックでも選べます）
- 再生画面の「設定」ボタン … BGM / SE / ボイスの音量、テキストウィンドウの透明度、テキストの速度、メッセージ送り時の音声継続（ゲームと同じ項目・同じ数値）
  - サイドバーの「マスターボリューム」が全体の音量、こちらが物語再生の要素ごとの調整です
