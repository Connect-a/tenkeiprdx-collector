...
function makeLocalQuad(sys, texByMatPid) {
  ...
  let alphaKeys = null;
  if (grad && grad.m_NumAlphaKeys) {
    alphaKeys = [];
    for (let i = 0; i < grad.m_NumAlphaKeys; i++) {
      const k = grad['key' + i];
      alphaKeys.push([(grad['atime' + i] || 0) / 65535, k && k.a != null ? k.a : 1]);
    }
    alphaKeys.sort((a, b) => a[0] - b[0]);
  }
  let colorKeys = null;
  if (grad && grad.m_NumColorKeys) {
    colorKeys = [];
    for (let i = 0; i < grad.m_NumColorKeys; i++) {
      const k = grad['key' + i];
      colorKeys.push([(grad['ctime' + i] || 0) / 65535, k ? k.r : 1, k ? k.g : 1, k ? k.b : 1]);
    }
    colorKeys.sort((a, b) => a[0] - b[0]);
  }
  const chAt = (f, idx) => {
    if (!colorKeys || !colorKeys.length) return 1;
    if (f <= colorKeys[0][0]) return colorKeys[0][idx];
    ...
    return colorKeys[colorKeys.length - 1][idx];
  };
  const alphaAt = (f) => {
...
class OrbitVelZ {
  ...
  initialize(p) {
    if (this.speed.startGen) this.speed.startGen(p.memory);
    if (!p._orbPrev) p._orbPrev = new THREE.Vector3();
    else p._orbPrev.set(0, 0, 0);
  }
  update(p) {
...
class ClampVelUnity {
  constructor(limitGen, dampen) {
    this.limit = limitGen;
    this.dampen = Number(dampen) || 0;
    this.type = 'ClampVelUnity';
  }
  initialize(p) {
    if (this.limit && this.limit.startGen) this.limit.startGen(p.memory);
  }
  update(p) {
    const v = p.velocity;
...
function buildParams(sys, texByMatPid, loop, geoResolver) {
  ...
  if (velMod.enabled && velMod.orbitalZ && (velMod.orbitalZ.scalar || velMod.orbitalZ.minScalar)) {
    const P = sys.pos || { x: 0, y: 0, z: 0 },
      qr = sys.rot || { x: 0, y: 0, z: 0, w: 1 },
      sc = sys.scale || { x: 1, y: 1, z: 1 };
...
const _SV_AXX = new THREE.Vector3(1, 0, 0),
  _SV_AXY = new THREE.Vector3(0, 1, 0),
  _SV_AXZ = new THREE.Vector3(0, 0, 1);
const _svqx = new THREE.Quaternion(),
  _svqy = new THREE.Quaternion(),
  _svqz = new THREE.Quaternion();
...
function createOverlay(canvas, opt0) {
  ...
  function ensureBloom(w, h) {
    ...
    if (!bloom) {
      ...
      const threshMat = new THREE.ShaderMaterial({
        uniforms: { t: { value: null }, threshold: { value: 0.9 } },
      ...
      const glowMat = new THREE.ShaderMaterial({
        uniforms: { t: { value: null }, intensity: { value: 1.0 } },
        vertexShader: FS_VERT,
  ...
  function renderBloom(opaqueBg) {
    ...
    b.copyMat.uniforms.t.value = b.rtScene.texture;
    drawFs(b.copyMat);
    b.glowMat.uniforms.t.value = b.rtA.texture;
    drawFs(b.glowMat);
  ...
  function play(bytes, texByMatPid, durMs, opt) {
    ...
    const mount = (g) => {
      if (orbit && pivot) pivot.add(g);
      else scene.add(g);
    };
    const buildFx = () => {
      fx = createSceneVfx(bytes, { texByMatPid, loop });
      if (fx) mount(fx.group);
      return fx;
    };
    if (!buildFx()) return;
    if (stage) {
    ...
    const rebuildFx = () => {
      try {
        if (orbit && pivot) pivot.remove(fx.group);
        else scene.remove(fx.group);
        fx.dispose();
      } catch (e) {}
      buildFx();
