...
// VFX 手続き型テクスチャ生成（skillエフェクト/オーラ共有）。
// 手続き型 Shader Graph の形状を DXBC 逆アセンブル/GLSL 復号由来の式で再生成する。
// 未実装 shader は softRadial(放射状ソフト粒)にフォールバック。
...
export function proceduralTex(proc) {
  ...
  if (sh === 'Shader Graphs/CircleHole_add') {
    ...
    fnUV = (ux, uy) => {
      ...
      for (const c of C) { const d = 2 * Math.hypot(ux - c[0], uy - c[1]); cover = Math.max(cover, Math.max(0, Math.min(1, (1 - d) / w))); }
      return 1 - cover;
    };
    ...
    // Shader Graphs/MagLineCross の WebGL GLSL 実装をそのまま移植(gold の ✨)。
    // 縦線 = (1-(1.4·|uv-(.5,0)|)^20)·(1-(1.4·|uv-(.5,1)|)^20)、横線 = 同(uv-(0,.5)),(uv-(1,.5))。
    // cross = max(縦,横)、中心グロー = clamp((1-(1.94·|uv-(.5,.5)|)^0.57)·1.47,0,1)。α = clamp(cross·glow,0,1)。
    fnUV = (ux, uy) => {
      const dv1 = Math.hypot(ux - 0.5, uy - 0), dv2 = Math.hypot(ux - 0.5, uy - 1);
      const vert = (1 - Math.pow(1.4 * dv1, 20)) * (1 - Math.pow(1.4 * dv2, 20));
      const dh1 = Math.hypot(ux - 0, uy - 0.5), dh2 = Math.hypot(ux - 1, uy - 0.5);
